function [index]= STA_LTA_now(depth_to_go, p, P_wave_arrival_time)
...
% n_alpha = sqrt((1./vp_now(1:end-1)).^2 - p^2);
% n_beta = sqrt((1./vs_now(end)).^2 - p^2);
% 
% del_t = sum(n_alpha.* th_now(1:end-1)) + (n_beta.* th_now(end))  ;
...
Arr_time_at_depth = P_wave_arrival_time - del_t;

index = round((Arr_time_at_depth * 20) + 1); % index has to be an integer. 
% Hence the use of round. The "+ 1" is necessary to include zero time.
